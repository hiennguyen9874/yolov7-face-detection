...
class WingLoss(nn.Module):
    def __init__(self, w=10, e=2):
        ...
        self.w = w
        self.e = e
        self.C = self.w - self.w * math.log(1 + self.w / self.e)
...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        tcls, tbox, tlmks, tlmks_mask, indices, anchors = self.build_targets(p, targets)  # targets
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            lmks = t[:, 6:16]
            lmks[:, [0, 1]] = lmks[:, [0, 1]] - gij
            lmks[:, [2, 3]] = lmks[:, [2, 3]] - gij
            lmks[:, [4, 5]] = lmks[:, [4, 5]] - gij
            lmks[:, [6, 7]] = lmks[:, [6, 7]] - gij
...
class ComputeLossOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model
        ...
        pre_gen_gains_lmks = [
            torch.tensor(pp.shape, device=device)[[3, 2, 3, 2, 3, 2, 3, 2, 3, 2]] for pp in p
        ]
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                selected_tlmks[:, [2, 3]] -= grid
                selected_tlmks[:, [4, 5]] -= grid
    ...
    def build_targets(self, p, targets, imgs):
        ...
        indices, anch = self.find_3_positive(p, targets[..., :6])
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            for i, pi in enumerate(p):
                ...
                p_cls.append(fg_pred[:, 5 : self.nc + 5])
