...
def check_requirements(requirements="requirements.txt", exclude=()):
    ...
    for r in requirements:
        try:
            ...
            n += 1
            print(
                f"{prefix} {e.req} not found and is required by YOLOV7-FACE, attempting auto-update..."
            )
...
def scale_coords_lmks(img1_shape, lmks, img0_shape, ratio_pad=None):
    if ratio_pad is None:  # calculate from img0_shape
        gain = min(
            img1_shape[0] / img0_shape[0], img1_shape[1] / img0_shape[1]
        )  # gain  = old / new
...
def bbox_iou(
    box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, eps=1e-7
):
...
def non_max_suppression_lmks(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float(), lmks), 1)
        else:  # best class only
            lmks = x[:, 5 + nc :]
            conf, j = x[:, 5:6].max(1, keepdim=True)
            x = torch.cat((box, conf, j.float(), lmks), 1)[conf.view(-1) > conf_thres]
